...
sub fileToArray ($) {
	...
	if ( $isLabel != 0 ) {
		for (my $r = 0; $r < $rows; $r++) {
			$numericLabels = 0 if !isNumeric($values[$r*$cols]);
		}
	} else {
		$numericLabels = 0;
	}
	setParam("_isLabelNumeric", $numericLabels);

	# Find extreme x values for hard scaling (Dimension Too Large LaTeX error)
	my $xmin;
	my $xmax;
	if ( $isLabel != 0 && $numericLabels == 1 ) {
		...
		# If no label or non-numeric label, takes number of rows
		$xmin = 1;
		$xmax = $rows;
	}
	...
	# Hard-scaling values
	if ( $xHardScale > 1 && $isLabel != 0  && $numericLabels) {
		for (my $r = 0; $r < $rows; $r++) {
			$values[$r*$cols] /= $xHardScale;
		}
	}
	if ( $yHardScale > 1 ) {
		for (my $r = 0; $r < $rows; $r++) {
